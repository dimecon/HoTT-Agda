...
module opetopes.simple.Opetopes where
  ...
  eric M i c δ ε = ((i , μ M (μ M c δ') ε') , γ) , b
    ...
    where δ' : (p : Pl M c) → Op M (Ty M p)
          ...
          τ : SlOp M (i , μ M (μ M c δ') ε')
          τ = box (μ M c δ') ε' (λ p → box (ε p) (λ q → η M (Ty M q)) (λ q → dot (Ty M q)))

          γ : SlOp M (i , μ M (μ M c δ') ε')
          γ = μ (Slice M) τ (consBox M σ 
                            (λ p → consBox M (η (Slice M) (Ty M p , ε p)) 
                            (λ q → consDot M (Ty M q))))
                             
          b : SlOp (Slice M) ((i , μ M (μ M c δ') ε') , γ)
          b = box τ φ (nodeElim M (λ p → SlOp (Slice M) (Ty (Slice M) p , φ {!!})) {!!} {!!})

                  -- (p : Pl (Slice M) τ) → SlOp (Slice M) (Ty (Slice M) p , φ p)
                  where φ = (consBox M σ 
                              (λ p → consBox M (η (Slice M) (Ty M p , ε p)) 
                              (λ q → consDot M (Ty M q))))

                  -- (λ { (this _ _ _) → η (Slice (Slice M)) ((i , μ M c δ') , σ) ; 
                  --      (that _ _ _ p (this _ _ _)) → η (Slice (Slice M)) ((Ty M p , ε p) , η (Slice M) (Ty M p , ε p)) ; 
                  --      (that _ _ _ p (that _ _ _ p₁ ())) 
                  --    })
